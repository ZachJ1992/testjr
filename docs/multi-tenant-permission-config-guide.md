...
| **「组织管理 → 新增组织」表单缺类型/关联实体字段** | ❌ 只能建出 `type=platform` 的空壳节点，**不要**在这里建主体 | 仅当业务方提出"在组织管理也能直接建主体"再做。**当前推荐流程：所有资金方/融资方主体一律通过档案页新建（见下行 ✅）** |
| 资金方/融资方档案 ↔ 组织节点 联动 | ✅ **已实现**：在「资源管理 → 资金方档案 / 融资方档案」新增、改名、删除时，会自动同步 `org_units` 节点（type、related_entity_id、name、is_active） | 保持现状 |
